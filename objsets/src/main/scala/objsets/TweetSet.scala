...
abstract class TweetSet {
  ...
  def descendingByRetweet: Trending = descendingByRetweet0(new EmptyTrending)
 
  def descendingByRetweet0(accu: Trending): Trending = {
    if (isEmpty) accu
    else {
      val min = findMin
      remove(min).descendingByRetweet0(accu + min)
    }
  }
